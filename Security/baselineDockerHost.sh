...
ADMINIP=$(grep "Accepted publickey" /var/log/auth.log | awk '{print $11}' | sed -e 's/[^0-9.a-z]*//g' -e '/^$/d' | sort | uniq)

echo ":: Setting up a firewall"
curl -s $FW_CONF > /etc/init/firewall.conf
curl -s $FW_POLICY > /etc/init.d/firewall
